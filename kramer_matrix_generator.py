...
def sequence_caption_generator(input1, output1):
    b = "select scene " + hex(input1)[2:].upper() + " on control unit A" + str(output1+1)
    return b
...
def data_generator(input1, output1):
    d = ":A" + hex(input1)[2:].upper() + str(output1+1) + "\r "
...
def matrix_generator(height, width):
    ...
    for i in range(0, height):
        feedback_sequence_generator(height, i, width)
    return result
...
def reply_data_generator(output1, input1):
    e = "@0PXOT" + str(output1 + 1) + "SS" + str(input1 + 1) + "\r"
    # the following line will put the output in bytes
    e = bytes(e, 'utf-8')
    # this line transforms the command into HEX code
...
def reply_sequence_caption_generator(output1, input1):
    b = "Audio output channel " + str(output1 + 1) + " Zone " + str(input1 + 1)
    return b


def reply_generator(mapped_seq, input1, output1):
    d = "                <Reply Caption=\"" + str(reply_sequence_caption_generator(output1, input1)) + "\" Guid=\""
    d += str(sequence_name_generator()) + "\">\n"
    d += "                  <Data>" + str(reply_data_generator(output1, input1)) + "</Data>\n"
...
def question_generator(count):
    e = " 0?PXOT" + str(count+1) + "SS\r"   # <-------------- POSSIBLE CHANGE HERE
    # the following line will put the output in bytes
    e = bytes(e, 'utf-8')
    # this line transforms the command into HEX code
    e = binascii.hexlify(e)
...
def feedback_sequence_generator(width, count, realcount):
    ...
    result += "\" Caption=\"Zone " + str(count+1) + "\"" + " Mode=\"Pull\" UseHeaderFooter=\"True\">\n"
    ...
    result += "              <ReplyByteOffset Value=\"" + str(count*0) + "\" />\n"
    ...
    result += "              <Replies>\n"
    p = count
    for y in range(0, realcount):
            print("count : " + str(count) + " | width " + str(width) + " | realcount : " + str(realcount) + " | p : "
                  + str(p) + " | y : " + str(y))
            result += str(reply_generator(sequence_names[p], y, count))
            p += width
            print(p)
    result += "              </Replies>\n"
